...
def refresh_cookies():
    ...
    if current_time - last_cookie_refresh < 30:
        logger.info("Skipping cookie refresh - recently done")
        return
    
    try:
        logger.info("Refreshing cookies from NSE...")
        res = session.get('https://www.nseindia.com/option-chain', 
                         headers=HEADERS, 
                         timeout=15)
...
def home():
    ...
    return jsonify({
        'status': 'running',
        'service': 'NSE Proxy API',
        'version': '2.0',
        'endpoints': {
            'GET /': 'This info',
            'GET /option-chain?symbol=NIFTY': 'Fetch option chain'
        }
    })
...
def option_chain():
    ...
    if symbol not in valid_symbols:
        return jsonify({'error': f'Invalid symbol. Use: {", ".join(valid_symbols)}'}), 400
    ...
    try:
        ...
        return jsonify({
            'error': f'NSE returned {r.status_code}',
            'message': r.text[:300]
        }), r.status_code
